...
import {
  ...
  _getWidgetAvailabilityHour,
  _deleteAvailabilityTime,
  _deleteWidgetAvailabilityTime,
  _postAvailabilityTime,
  _postWidgetAvailabilityTime
...
class BusinessCommonWorkingHour extends Component {
  state = {
    activeToggle: false,
    working_from: '',
    working_to: '',
  ...
  handleToggleData = toggleData => {
    updateHourStatus(toggleData, this.props.widget.id)
      ...
      .catch(err => console.error(err, "error-msg") )
  }
  ...
  fetchAvailabilityHour = async () => {
    this.props.loading(true)
    const widget_id = await localStorage.getItem('widget_id');
    const url = `api/v1/widget/${widget_id}/working-hours`

    _getWidgetAvailabilityHour(url)
      .then(res => {
  ...
  onDeleteAvailabilityTime =async id => {
    this.props.loading(true)
    const widget_id = await localStorage.getItem('widget_id');
    const url = `api/v1/widget/${widget_id}/working-hours/${id}`
    _deleteWidgetAvailabilityTime(url)
      .then(() => {
        this.fetchAvailabilityHour()
  ...
  onPostWorkingHours = async () => {
    ...
    if (isCallAPI) {
      const url = `api/v1/widget/${widget_id}/working-hours`
      _postWidgetAvailabilityTime(url, fordata)
        .then(res => {
          this.onSaveBtnLoader(false)
          this.beforeFetch(res.data)
        ...
        <div>
          <div className="business_hours_main">
            <div className="working-hours-wrapper availability-block break-section">
              ...
              <div className="break-time-wrapper business-hour-table-container">
...
export default BusinessCommonWorkingHour
